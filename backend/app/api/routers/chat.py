...
BASE_SYSTEM_PROMPT = (
    ...
    "## 回答格式要求\n"
    "当用户请求分析、评估、规划或报告时，以结构化 Markdown 格式回复。"
    "章节标题根据用户问题动态确定，不强制使用固定六段式模板。\n\n"
    "## 正文生成要求\n"
    "所有分析内容必须基于用户的真实数据（学生画像、OCR 简历、岗位画像、匹配结果等）生成。\n"
    "禁止在回答中注入固定结论文案、通用建议或虚构内容。\n"
    "如果用户数据不足，明确告知缺少哪些信息，而不是编造通用分析。\n\n"
